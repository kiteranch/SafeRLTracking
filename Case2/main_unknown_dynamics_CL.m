...
Env.c1=[-2;0]; Env.r1a=0.7; Env.r1b=1; 
R=1;
Q=diag([10 10]);
...
%--------------------------------------------------------------------------
function [ydot,ra,u,mu,udhat,usafe,meP,B,h1]=closedLoopDynamics(t,y,n,L,p,QT,R,etac1,etac2,etaa1,etaa2,beta,nu, ...
...
h1 = (x(1)-c1(1))^2/r1a^2+(x(2)-c1(2))^2/r1b^2-1;
nabla_h1 = [2*(x(1)-c1(1))/r1a^2; 2*(x(2)-c1(2))/r1b^2];

cbf1=1/h1^3;
B = norm(e)^2*(cbf1);
gradxB = 2*e*(cbf1) + norm(e)^2*(-3*nabla_h1/h1^4);
%==========================================================================
usafe = -cb*g'*gradxB;
...
% h1d = (xd(1)-c1(1))^2/r1a^2+(xd(2)-c1(2))^2/r1b^2-1;
% if h1d<eps
%     fprintf('t=%.4f, xd not in Int(S) \n',t)
% end
